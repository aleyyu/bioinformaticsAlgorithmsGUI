...
class MainWindow(QMainWindow, Ui_MainWindow):
    def __init__(self):
        ...
        # DB BUTTON
        self.ui.showdbbttn.clicked.connect(self.loaddatafm)
        self.ui.showdbbttn.clicked.connect(self.loaddatakmer)
        self.ui.showdbbttn.clicked.connect(self.loaddatabloom)
        self.ui.showdbbttn.clicked.connect(self.loaddataaho)
        self.ui.showdbbttn.clicked.connect(self.loaddatasmith)

    #DB TABLE
    def loaddatafm(self):
        connection = sqlite3.connect('database.db')
        query = "SELECT * FROM FMINDEX"
        result = connection.execute(query)

        for row_number, row_data in enumerate(result):
            self.ui.fmdb.insertRow(row_number)
            for column_number, data in enumerate(row_data):
                self.ui.fmdb.setItem(row_number, column_number, QtWidgets.QTableWidgetItem(str(data)))
        connection.close()

    def loaddatakmer(self):
        connection = sqlite3.connect('database.db')
        query = "SELECT * FROM KMER"
        result = connection.execute(query)

        for row_number, row_data in enumerate(result):
            self.ui.kmerdb.insertRow(row_number)
            for column_number, data in enumerate(row_data):
                self.ui.kmerdb.setItem(row_number, column_number, QtWidgets.QTableWidgetItem(str(data)))
        connection.close()

    def loaddatabloom(self):
        connection = sqlite3.connect('database.db')
        query = "SELECT * FROM BLOOMFILTER"
        result = connection.execute(query)

        for row_number, row_data in enumerate(result):
            self.ui.bloomdb.insertRow(row_number)
            for column_number, data in enumerate(row_data):
                self.ui.bloomdb.setItem(row_number, column_number, QtWidgets.QTableWidgetItem(str(data)))
        connection.close()

    def loaddataaho(self):
        connection = sqlite3.connect('database.db')
        query = "SELECT * FROM CAROSICK"
        result = connection.execute(query)
        self.ui.ahodb.setRowCount(0)

        for row_number, row_data in enumerate(result):
            self.ui.ahodb.insertRow(row_number)
            for column_number, data in enumerate(row_data):
                self.ui.ahodb.setItem(row_number, column_number, QtWidgets.QTableWidgetItem(str(data)))
        connection.close()

    def loaddatasmith(self):
        connection = sqlite3.connect('database.db')
        query = "SELECT * FROM SMITHWATERMAN"
        result = connection.execute(query)

        header = self.ui.smithdb.horizontalHeader()
        header.setSectionResizeMode(QtWidgets.QHeaderView.ResizeToContents)

        for row_number, row_data in enumerate(result):
            self.ui.smithdb.insertRow(row_number)
            for column_number, data in enumerate(row_data):
                self.ui.smithdb.setItem(row_number, column_number, QtWidgets.QTableWidgetItem(str(data)))
        connection.close()
    ...
    def kmer_draw_graph_1(self):
        ...
        pen = pyqtgraph.mkPen(color=(255, 0, 0), width=1, style=QtCore.Qt.DotLine)
    ...
    def kmer_draw_graph_3(self):
        ...
        bargraph = pyqtgraph.BarGraphItem(x=x, height=y, width=0.6, brush='g')
        styles = {'color': 'r', 'font-size': '15px'}
